...
from networks.segtran_shared import SegtranConfig, bb2feat_dims, SegtranFusionEncoder, SegtranInitWeights, \
    gen_all_indices
...
class Segtran2dConfig(SegtranConfig):
    def __init__(self):
        ...
        self.backbone_type = 'resnet50'  # resnet50, resnet101, resibn101, eff-b1~b4
...
class Segtran2d(SegtranInitWeights):
    def __init__(self, config):
        ...
        if self.backbone_type.startswith('resnet'):
            ...
            # self.backbone = resnet.__dict__[self.backbone_type](pretrained=self.use_pretrained,do_pool1=not self.bb_feat_upsize)
    ...
    def forward(self, batch):
        ...
        if not self.scales_printed:
            print("\nImage scales: %dx%d. Feat: %s. Voxels: %s" % (
                scale_H, scale_W, list(xy_shape), list(vfeat_fpn.shape)))
