...
class SegtranConfig:
    ...
    def set_fpn_layers(self, config_name, fpn_settings, do_print=True):
        ...
        if self.out_fpn_layers[-1] > self.in_fpn_layers[-1]:
            print(
                "in_fpn_layers=%s is not compatible with out_fpn_layers=%s" % (self.in_fpn_layers, self.out_fpn_layers))
            exit(0)
    ...
    def forward(self, input_feat, attention_probs, in_geoshape=None):
        ...
        if self.num_scales > 0:
            ...
            for s in range(self.num_scales):
                ...
                mince_first_feat_fusion = resize_flat_features(mince_first_feat_fusion, scale_feat_shapes[s],
                                                               orig_geoshape=in_geoshape)
    ...
    def forward(self, in_query, in_key=None, pos_biases=None):
        ...
        if self.training:
            ...
            if self.call_count % self.attn_diag_cycles == 0:
                print("max-attn: {:.2f}, avg-attn: {:.2f}, clamp-count: {}".format(self.max_attn, curr_avg_attn,
                                                                                   self.clamp_count))
                self.max_attn = 0
                self.clamp_count = 0
            ...
            if i < MAX_POS_LAYER:
                ...
                if self.num_scales > 0:
                    ...
                    if self.pos_code_type == 'bias' or self.pos_code_type == 'none':
                        pos_code = [self.pos_code_layers[s](scale_feat_shapes[s], voxels_pos) for s in
